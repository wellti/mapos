...
class Os_model extends CI_Model
{
    ...
    public function autoCompleteProduto($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['descricao'] . ' | Estoque: ' . $row['estoque'], 'estoque' => $row['estoque'], 'id' => $row['idProdutos'], 'preco' => $row['precoVenda']];
            }
    ...
    public function autoCompleteCliente($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['nomeCliente'] . ' | Setor: ' . $row['rua'], 'id' => $row['idClientes']];
            }
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteUsuario($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['nome'] . ' | Ramal: ' . $row['telefone'], 'id' => $row['idUsuarios']];
            }
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteServico($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['nome'] . '' . $row[''], 'id' => $row['idServicos'], ];
            }
            echo json_encode($row_set);
        }
    }
